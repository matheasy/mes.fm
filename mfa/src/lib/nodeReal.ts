 ...
 *
 * CAVEAT: NodeReal doesn't publish a full worked example of `nr_getAssetTransfers`'s response
 * shape, or documented free-tier rate limits. The shape assumed below (`{ transfers: [...],
 * pageToken }`, per-transfer `value`/`rawContract`/`metadata` fields) is inferred from NodeReal's
 * own migration-guide example plus its close resemblance to Alchemy's `alchemy_getAssetTransfers`
 * - though it diverges from Alchemy on at least one confirmed point: `value` is raw integer
 * units, not decimal-adjusted (see ledger.ts's use of `formatUnits`). `paginateTransfers()` throws
 * a clear error if the response isn't shaped as expected, rather than risk silently-wrong numbers.
 */
...
export interface AssetTransfer {
  ...
  from: string;
  to: string | null;
  /** Raw integer units (confirmed live), NOT decimal-adjusted despite resembling Alchemy's API - scale by decimals before use */
...
async function paginateTransfers(direction: 'fromAddress' | 'toAddress', address: string): Promise<AssetTransfer[]> {
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    if (!result || !Array.isArray(result.transfers)) {
      throw new Error('NodeReal nr_getAssetTransfers returned an unexpected response shape (see nodeReal.ts caveat)');
    }
