...
<body>
	...
	<p>PERCENTAGE CALCULATOR MEMES</p>
	...
	<table id="myTable">
		<tr>
			...
			<th>ID</th>
			<th>MEME LINK</th>
			<th>UPLOAD TYPE (1=memes, 2=interesting facts)</th>
		</tr>
		...
		while($row = mysqli_fetch_array($result)) {
		  ...
		  '<td><div><img src = "https://percentagecalculator.mes.fm/img/memes-thumbnail/'.$row['thumbnailurl2'].'" /></td>'.
		  '<td>'.$row['thumbnailurl2'].'</td>'.
		  '<td>'.$row['id'].'</td>'.
		  '<td><a href="https://percentagecalculator.mes.fm/'.($row['uploadtype']=='1'?'memes':'interesting-facts').'/'.$row['urlname'].'" target="_blank">https://percentagecalculator.mes.fm/'.($row['uploadtype']=='1'?'memes':'interesting-facts').'/'.$row['urlname'].'</a></td>'.
