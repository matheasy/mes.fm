...
<body>
	...
	<table id="myTable">
		...
		while($row = mysqli_fetch_array($result)) {
		  ...
		  '<img src = "https://timer.mes.fm/img/memes-thumbnail/'.$row['thumbnailurl2'].'" /></td>'.
		  '<td>'.$row['thumbnailurl2'].'</td>'.
		  '<td>'.$row['id'].'</td>'.
		  '<td><a href="https://timer.mes.fm/inspirational-quotes/'.$row['urlname'].'" target="_blank">https://timer.mes.fm/inspirational-quote/'.$row['urlname'].'</a></td>'.
		...
		$(".save").click(function() {
			...
			if(makeChanges) {
				$.post('Dont_Touch/Save_Meme.php', { 
					...
					desc: desc,
					id: id,
					calc: 4 //1 = gc, 2 = bmi, 3 = pc, 4 = timer
				},
