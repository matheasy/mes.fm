...
// php/getShareUrl.php) which read/wrote a MySQL table (sharecalcdb.pc)
...
// into this project's Vercel settings, as plain project-level variables --
// Shared Environment Variables silently failed to actually reach the
// function at runtime for every other calculator site, see
// inflationcalculator.mes.fm's commit 3d2ccabb) -- no new database to
// provision, just a "share:pc:<id>" string key per share. Same pattern as
// the other 6 calculator sites' api/share.js.
//
// POST { data: {...} }        -> { id }
// GET  ?id=<id>                -> the original equation object
const crypto = require('crypto');

const KEY_PREFIX = 'share:pc:';
