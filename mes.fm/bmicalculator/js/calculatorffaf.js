$(document).ready(function(){
		...
		return {
			...
			calcFinalResult: function(weight,height,bmi) {
				...
				switch(true)
				{
				  ...
				  case bmi > 29.9:
				    ...
				    break;
				  default:
				    finalResult.text("Please enter your weight and height in the fields to the left.");
			...
			shareUrl: function () {
				...
				if($("#bmi-input-result").val() == '') {
					$("#url-to-share").val("https://mes.fm/bmicalculator");
					return false;
				}
			...
			createUrl: function(data) {
				...
				// php/createShareUrl.php used to write to a MySQL table (sharecalcdb.bmi)
				...
				// mes.fm/api's own pageview tracking already uses; ?calc=bmi namespaces
				// the key). Share links are now mes.fm/bmicalculator/s/<id>.
				$.ajax({
					url:'/api/share?calc=bmi',
					type:'POST',
					contentType:'application/json',
					data: JSON.stringify({calc:'bmi', data:data})
				})
					.done(
						function(response) {
							var url = window.location.origin + '/bmicalculator/s/' + response.id;
			...
			initializeCustomCalculation: function() {
				...
				var match = window.location.pathname.match(/^\/bmicalculator\/s\/([A-Za-z0-9]+)\/?$/);
	...
	$("#pounds-input,#feet-input,#inches-input").each(function() {
	    var currentVal = $(this);
  	...
  	$("#kilograms-input,#centimeters-input").each(function() {
    	var currentVal = $(this);
    	// Save current value of input
    	currentVal.data('currentVal', currentVal.val());
    	// Look for changes in the value
