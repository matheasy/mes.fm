	...
	var CALCULATOR = (function() {
		...
		return {
			...
			createUrl: function(data) {
				...
				// mes.fm/api's own pageview tracking already uses; ?calc=mc namespaces
				// the key). Share links are now mes.fm/mortgagecalculator/s/<id>.
				$.ajax({
					url:'/api/share?calc=mc',
					type:'POST',
					contentType:'application/json',
					data: JSON.stringify({calc:'mc', data:data})
				})
					.done(
						function(response) {
							var url = window.location.origin + '/mortgagecalculator/s/' + response.id;
			...
			initializeCustomCalculation: function() {
				...
				var match = window.location.pathname.match(/^\/mortgagecalculator\/s\/([A-Za-z0-9]+)\/?$/);
