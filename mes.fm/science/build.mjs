...
<html>
<head>
  <link rel="canonical" href="https://mes.fm/science" />
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="description" content="MES Science - Links, videos, and posts on science topics investigated by MES, including cold fusion, ferrocell optics, vortex math, biology, and virology.">
  <meta name="keywords" content="MES, Science, Math Easy Solutions">
  <meta name="author" content="Mathiew Estepho">
  <link rel="icon" href="https://mes.fm/img/science-logo.png?v=1.0" type="image/png" />
  <meta property="og:type" content="website" />
  <meta property="og:url" content="https://mes.fm/science" />
  <meta property="og:title" content="MES Science" />
  <meta property="og:description" content="Links, videos, and posts on science topics investigated by MES, including cold fusion, ferrocell optics, vortex math, biology, and virology." />
  <meta property="og:image" content="https://mes.fm/img/science-logo-big.png" />
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:image" content="https://mes.fm/img/science-logo-big.png">
  <meta name="twitter:image:alt" content="MES Science">
  <title>MES Science Tutorials</title>
  <style>
    * { box-sizing: border-box; }
    ...
    /* Wider band for the header + the Posts/Videos card grids. The flat link
       list below stays in .content at a readable measure. */
    .wide {
      max-width: 1180px;
      margin: 0 auto;
      padding: 0 1.25em;
    ...
    .content { font-size: calc(1rem * var(--ts, 1)); }

    .sub-heading { font-size: 1.1em; cursor: pointer; }
    .arrow-icon { font-size: 20px; display: inline-block; vertical-align: middle; transition: transform 0.3s ease; }
    .hidden { display: none; }
    .collapsible-section { margin-bottom: 1.5em; }

    /* Topic-homepage card grid — see buildCardGrid() above. Kept in sync with
       the identical block in mes.fm/911/build.mjs, mes.fm/hutchison/build.mjs,
       and mes.fm/conspiracy/build.mjs. */
    .card-grid {
      display: grid;
    ...
    .link-card-title { font-size: 0.95rem; font-weight: 700; line-height: 1.3; color: inherit; }
    .link-card-excerpt { font-size: 0.8rem; opacity: 0.7; line-height: 1.4; flex: 1; color: inherit; }
    .link-card-readmore { font-size: 0.8rem; font-weight: 600; }
    ...
    /* Shared mes.fm site navigation: hamburger + dropdown (#navbar-button /
       ...
       rules don't win. Science's own amber/orange accent (#b6570f light /
       #ffb066 dark), matching the atom-themed logo. */
    .top-bar {
      position: relative;
      display: flex;
      align-items: center;
    ...
    body.light .hamburger-btn { color: #b6570f; }
    body.dark .hamburger-btn { color: #ffb066; }
    #navbar { display: none; }
    #navbar.hide {
      display: flex; flex-direction: column; box-sizing: border-box;
      position: absolute; top: 100%; right: 0; z-index: 30;
      width: 16em; max-width: 90vw; max-height: 75vh; overflow-y: auto;
      background-color: #b6570f; border-radius: 0.25em;
    ...
    #navbar .navbar__link:hover:not(.navbar__dropdown-item):not(.active-tab) { background-color: #8c430c; }
    ...
    .info-bar { display: flex; flex-wrap: wrap; background-color: #b6570f; box-shadow: 0.2em 0.2em 0.5em rgba(0, 0, 0, 0.35); }
    ...
    #info-bar .info-bar__item__text:hover:not(.active-tab) { background-color: #8c430c; }
    ...
    /* Site footer -- same links/markup as mes.fm/math and mes.fm/911, in
       science's own amber/orange accent. Colors use ".footer .x" (0,2,0) so
       the body.light/body.dark "a" rules don't win. */
    .footer { max-width: 1180px; margin: 2.5em auto 0; padding: 2em; background-color: #b6570f; border-radius: 0.35em; }
    ...
    /* Floating compact header bar: once the real top-bar + nav scroll out of
       ...
       right end. Ported from mes.fm/math's version (see its build.mjs) and
       mes.fm/911's -- this page's top-bar is already a clean flex row (no
       position:absolute/table-cell legacy layout to fight).
    ...
    #compact-nav {
      ...
      background: #ffffff; border-bottom: 3px solid #b6570f;
    ...
    #compact-nav .compact-nav-links a:hover { background: rgba(182,87,15,0.12); }
    ...
    body.light #compact-nav .compact-nav-links a { color: #b6570f; }
    body.dark #compact-nav .compact-nav-links a { color: #ffb066; }
...
/* autoads-header-gap-guard: guards two gaps against Google Auto ads (in-page ad blocks,
   ...
   block did -- no relocate/poll needed, just hide on sight.
   In the header/nav case Google's own placeholder-collapse doesn't reliably
...
(function () {
    ...
    function handleHeaderNavZone(node) {
        ...
        var target = document.querySelector('.wide');
        if (target) {
            target.before(node);
        } else if (nav) {
            nav.after(node);
    ...
    function inAnnotationGuardedZone(el) {
        return !!(el.closest('.top-bar') || el.closest('#footer') || el.closest('.info-bar-container'));
    }
  ...
  <div class="wide">
    <div class="top-bar">
      <div class="site-brand">
        <a class="site-brand-logo-link" href="/science"><img class="site-brand-logo" src="/img/science-logo.png" width="88" height="88" alt="MES Science logo"></a>
        <div class="site-brand-text"><a class="site-brand-title" href="/science">MES Science</a><p class="site-brand-tag">Links, videos and posts on science topics.</p></div>
      </div>
    ...
    <div class="info-bar-container" role="navigation" aria-label="Primary">
      <ul id="info-bar" class="info-bar">
        ...
        <li class="info-bar__item"><a target="_self" class="info-bar__item__text" href="/math">Math Tutorials</a></li>
...
${videosGridHtml}
  ...
  <script>
    // text-size control: same STEPS / 'articleFontScale' key as mes.fm/math
    // and mes.fm/911. Drives the --ts CSS variable, which only .page-lede
    // multiplies into its font-size here (this page has no long-form prose).
    (function () {
  ...
  <script>
    ...
    // #compact-nav. Identical to mes.fm/math's and mes.fm/911's version.
    (function () {
