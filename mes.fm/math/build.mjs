...
/* Floating compact bar: once the full header + nav scroll out of view, a slim
   ...
   display controls + hamburger are pinned into its right end. Toggled by
   body.is-stuck (see the sticky-bar script). */
#sticky-bar {
  ...
  transform: translateY(-110%); transition: transform 0.2s ease; visibility: hidden;
}
body.is-stuck #sticky-bar { transform: none; visibility: visible; }
#sticky-bar .sticky-logo { display: block; width: 32px; height: 32px; border-radius: 6px; flex: 0 0 auto; }
#sticky-bar .sticky-title {
  flex: 0 1 auto; min-width: 0; font-weight: 700; font-size: 1.05em; color: #1a1a1a;
  text-decoration: none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
#sticky-bar .sticky-links { display: flex; flex: 1 1 0; min-width: 0; overflow: hidden; margin: 0 0 0 8px; padding: 0; list-style: none; }
#sticky-bar .sticky-links li { flex: 0 0 auto; }
#sticky-bar .sticky-links a { display: block; padding: 0 10px; line-height: 52px; font-size: 0.9em; color: #277bb6; text-decoration: none; white-space: nowrap; }
#sticky-bar .sticky-links a:hover { background: rgba(39,123,182,0.12); }
body.dark-mode #sticky-bar { background: #1a1a1a; }
body.dark-mode #sticky-bar .sticky-title { color: #eeeeee; }
body.dark-mode #sticky-bar .sticky-links a { color: #6cb6f5; }
...
@media (max-width: 700px) {
  #sticky-bar .sticky-title { display: none; }
  #sticky-bar .sticky-logo { width: 36px; height: 36px; }
}
@media (max-width: 480px) {
  #sticky-bar { padding-right: 160px; gap: 8px; }
...
<div id="sticky-bar" aria-hidden="true">
  <a href="/math" tabindex="-1"><img class="sticky-logo" alt="" width="32" height="32" src="https://mes.fm/img/logo-mark.png"></a>
  <a class="sticky-title" href="/math" tabindex="-1">MES Math Tutorials</a>
...
<script>
  // Floating compact bar (see #sticky-bar CSS): appears once the header + nav
  // bar have scrolled off the top; nav links that don't fit are dropped from
  // the right, least important first, rather than wrapping.
  (function () {
    var bar = document.getElementById('sticky-bar');
    var nav = document.querySelector('.info-bar-container');
    var links = bar && bar.querySelector('.sticky-links');
