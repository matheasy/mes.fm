// Build-time generator for the "Posts" section of mes.fm/crypto.
//
...
// Posts thumbnail-card grid (scraping each linked page's og: tags,
...
// add a Post or want to refresh thumbnails/excerpts, then commit the
...
// Committed cache of link metadata scraped from each Posts card target
...
// "Posts" -- a hand-maintained list of mirrored mes.fm pages that
...
// link-meta.json.
const POSTS = [
  { href: "https://mes.fm/cadd-stablecoin-solana", title: "Canada's First Stablecoin $CADD Is Live on Solana" },
  { href: "https://mes.fm/3speak-ad-revenue-sharing", title: "3Speak Now Pays Users a 10% Share of Ad Revenue for Watching" },
  ...
  { href: "https://mes.fm/alex-jones-xrp-bitcoin", title: "Crypto News: Alex Jones on Your $XRP and a $320M $BTC Hack" },
...
<head>
  ...
  <style>
    ...
    /* .card-grid (display:grid, defined further down) has the same specificity
       as a bare .hidden and wins on source order, so the Posts grid never
    ...
    /* Wide band for the Posts card grid -- reads bigger than the
...
async function main() {
  console.log(`Resolving card metadata for ${POSTS.length} links ...`);
  const meta = await resolveAllMeta(POSTS);
