// Build-time generator for mes.fm/norman-patricia-ai-email.
//
// Fetches @mestruth/911-jersey-girl-patricia-casazzas-bizarre-ai-email-ddr from
// Hive's public bridge API and writes a static index.html. This is NOT run by
// Vercel -- run it manually (`npm run build`) whenever the Hive article
// changes, then commit index.html.
//
// Based on the mes.fm/ferrocell-specular-reflection scaffold (collapsible
// chapters + "Jump to" table of contents), with two differences specific to
// this article's structure:
//   * every "## " AND "### " heading becomes its own chapter (not just "# "),
//     since this article's 14 numbered evidence points are "## N. Title"
//     headings, not "# " ones.
//   * chapters default to open except "To be added to the list later when I
//     have time", which starts collapsed (see DEFAULT_COLLAPSED_CHAPTERS).
// It also adds a "Collapse All" control inside the sidebar TOC itself, not
// just the toolbar above the article body.
//
// Usage:
//   npm install
//   npm run build
...
const PERMLINK = "911-jersey-girl-patricia-casazzas-bizarre-ai-email-ddr";
const COMMUNITY = "hive-113182";
const PEAKD_URL = `https://peakd.com/${COMMUNITY}/@${AUTHOR}/${PERMLINK}`;
const CANONICAL = "https://mes.fm/norman-patricia-ai-email";
const BACK_LINK = "https://mes.fm/911";
const BACK_LABEL = "&larr; mes.fm/911";

// Chapter titles (exact, post-HTML-stripping text) that should render
// collapsed by default instead of open.
const DEFAULT_COLLAPSED_CHAPTERS = new Set([
  "To be added to the list later when I have time",
]);
...
function checkSlugMatch(post) {
  ...
  const selfRef = post.body.includes(CANONICAL) || post.body.includes(slug);
  const otherMesLinks = [
    ...new Set(post.body.match(/mes\.fm\/[a-z0-9-]+/gi) || []),
  ].filter((u) => !slug.startsWith(u));
  const status = selfRef
    ? "(OK — article self-references this page)"
    : otherMesLinks.length
    ? `(NO self-reference; body links other mes.fm pages: ${otherMesLinks.join(", ")})`
    : "(no mes.fm links in the body — nothing to check against)";
  console.log(`Slug check: @${AUTHOR}/${PERMLINK} "${post.title}" -> ${CANONICAL} ${status}`);
  if (!selfRef && otherMesLinks.length && process.env.ALLOW_SLUG_MISMATCH !== "1") {
    throw new Error(
      `\n\n  ⚠  SLUG MISMATCH\n` +
        `  The article @${AUTHOR}/${PERMLINK} ("${post.title}")\n` +
        `  never links to ${CANONICAL}, but does link other mes.fm pages\n` +
        `  (${otherMesLinks.join(", ")}). PERMLINK and CANONICAL may be out of sync --\n` +
        `  double-check the Hive URL. Re-run with ALLOW_SLUG_MISMATCH=1 to build anyway.\n`
    );
  }
}
...
// Bare YouTube URLs on their own line become a responsive iframe embed with a
// Theater Mode toggle + badge.
...
// table-of-contents can link to it. Chapter headings here are every "## " AND
// "### " heading (rendered as <h2>/<h3> by marked) -- this article's structure
// is one intro "# " heading followed by sixteen "## " sections and a single
// "### " one, and every "## "/"### " needs to be its own jump-to/collapsible
// chapter (not just a styled subheading). Both levels are promoted to <h1> so
// wrapChaptersInToggles can treat them uniformly. Duplicate slugs/labels get a
// "(2)"-style suffix.
function addSectionAnchors(bodyHtml) {
  const seen = new Map();
  const toc = [];
  const promoted = bodyHtml
    .replace(/<h2>([\s\S]*?)<\/h2>/g, "<h1>$1</h1>")
    .replace(/<h3>([\s\S]*?)<\/h3>/g, "<h1>$1</h1>");
...
// Each section runs from its own "<h1 id=\"...\">" (added by addSectionAnchors)
// until the next one (or the end of the body). Wrap each heading + content in a
// chapter-toggle div so it can be collapsed via toggleChapter(); a fresh <hr>
// is emitted before each as the visual divider. Chapters whose plain title is
// in DEFAULT_COLLAPSED_CHAPTERS start collapsed; every other chapter starts
// open.
function wrapChaptersInToggles(html) {
  const re = /<h1 id="([^"]+)">([\s\S]*?)<\/h1>/g;
  ...
  matches.forEach((m, i) => {
    ...
    const plainTitle = cleanTitle.replace(/<[^>]+>/g, "").trim();
    const collapsed = DEFAULT_COLLAPSED_CHAPTERS.has(plainTitle);
...
// Mirrored post images use alt="" (no descriptive alt text), matching every
// other mirror page in this repo -- marked otherwise carries the markdown
// image label (a generic "image.png" / Telegram filename) into alt.
function blankImageAlt(html) {
  return html.replace(/(<img\b[^>]*\balt=")[^"]*(")/g, "$1$2");
}

async function buildPage(post) {
  const title = post.title;
  const preprocessed = embedYoutubeLinks(post.body);
  const { html: parsedBodyHtml, toc } = addSectionAnchors(blankImageAlt(marked.parse(preprocessed)));
  const wrappedBodyHtml = wrapChaptersInToggles(parsedBodyHtml);

  // Everything before the first "## "/"### " heading (the intro paragraphs +
  // images, plus the "# The Culprit..." heading's own intro/mini-TOC, since
  // "# " isn't promoted -- it stays as the plain top-of-article <h1>) becomes
  // its own collapsible "Overview" chapter, pinned above the article's own
  // sections.
...
`;
  ...
  const description =
    ...
    "9/11 Jersey Girl widow Patricia Casazza sent MES a bizarre AI-generated email that was BCCed to many 9/11 disinfo spooks -- evidence points to 9/11 Revisionist Norman Swanepoel having written it with AI. Mirrored from the Hive blockchain.";
  const ogImage =
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    /* The "Collapse All" / "Expand All" chapter-toolbar button borrows
       .theme-toggle-btn for its base look but must NOT inherit the round
       sun/moon icon styling -- keep it the plain pill button it was before. */
    #toggleAllChaptersBtn {
      width: auto;
      height: auto;
      padding: 5px 10px;
      border: none;
    ...
    body.light #toggleAllChaptersBtn { background-color: #dddddd; color: #000000; border: none; }
    body.dark #toggleAllChaptersBtn { background-color: #444444; color: #ffffff; border: none; box-shadow: none; }
    ...
    /* Every chapter (the leading "Overview" one plus every Hive-sourced
       "## "/"### " section) is a collapsible dropdown -- see
...
<body class="dark">
  ...
  <div class="container">
    <div class="top-bar">
      <a class="site-link" href="${BACK_LINK}">${BACK_LABEL}</a>
...
async function main() {
  ...
  execFileSync("python3", [join(__dirname, "..", "..", "convert_mirror_pages.py"), "--apply", "--only=norman-patricia-ai-email"], { stdio: "inherit" });
