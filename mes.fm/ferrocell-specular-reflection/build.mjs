// Build-time generator for mes.fm/ferrocell-specular-reflection.
//
// Fetches @mes/demystifying-the-ferrocell-specular-reflection from Hive's public
// bridge API and writes a static index.html. This is NOT run by Vercel -- run it
// manually (`npm run build`) whenever the Hive article changes, then commit
// index.html.
//
...
const AUTHOR = "mes";
const PERMLINK = "demystifying-the-ferrocell-specular-reflection";
const COMMUNITY = "hive-128780";
const PEAKD_URL = `https://peakd.com/${COMMUNITY}/@${AUTHOR}/${PERMLINK}`;
const CANONICAL = "https://mes.fm/ferrocell-specular-reflection";
const BACK_LINK = "https://mes.fm/science";
...
// until the next one (or the end of the body) -- this article's "# " headers
// aren't preceded by "---", so we don't require a leading <hr>. Wrap each
// heading + content in a chapter-toggle div so it can be collapsed via
// toggleChapter(); a fresh <hr> is emitted before each as the visual divider.
...
`;
  ...
  const description =
    ...
    "Demystifying the ferrocell: the holographic colored lines are specular reflection off nanoparticles aligned along the magnetic field, not the field lines themselves -- with tutorial and simulation videos. Mirrored from the Hive blockchain.";
  const ogImage =
...
<body class="dark">
  ...
  <div class="container">
    <div class="top-bar">
      <a class="site-link" href="${BACK_LINK}">&larr; mes.fm/science</a>
...
async function main() {
  ...
  execFileSync("python3", [join(__dirname, "..", "..", "convert_mirror_pages.py"), "--apply", "--only=ferrocell-specular-reflection"], { stdio: "inherit" });
