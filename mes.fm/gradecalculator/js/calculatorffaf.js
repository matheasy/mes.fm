		...
		return {
			...
			shareUrl: function () {
				...
				if($("#desired-grade-input").val() == '' || $("#current-grade-input").val() == '' || $("#final-weight-input").val() == '') {
					$("#url-to-share").val("https://mes.fm/gradecalculator");
					return false;
				}
			...
			createUrl: function(data) {
				...
				// mes.fm/api's own pageview tracking already uses; ?calc=gc namespaces
				// the key). Share links are now mes.fm/gradecalculator/s/<id>.
				$.ajax({
					url:'/api/share?calc=gc',
					type:'POST',
					contentType:'application/json',
					data: JSON.stringify({calc:'gc', data:data})
				})
					.done(
						function(response) {
							var url = window.location.origin + '/gradecalculator/s/' + response.id;
			...
			initializeCustomCalculation: function() {
				...
				var match = window.location.pathname.match(/^\/gradecalculator\/s\/([A-Za-z0-9]+)\/?$/);
